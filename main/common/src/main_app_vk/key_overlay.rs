    ...
    fn new(renderer: &Renderer, setting: &Setting) -> Self {
        ...
        let (key_bar, press_rect, static_overlay, numbers) = std::thread::scope(|s| {
            ...
            let static_overlay_create_thread = {
                ...
                let properties_buffer = properties_buffer.clone();
                s.spawn(|| {
                    StaticOverlayShader::new(
                        queue,
                        render_pass,
                        allocators,
                        screen_size,
            ...
            let key_bar = KeyBarShader::new(
                renderer,
                screen_size,
                uniform_buffer.clone(),
                properties_buffer.clone(),
            );
            let press_rect = PressRectShader::new(
                renderer,
                screen_size,
                uniform_buffer.clone(),
                properties_buffer.clone(),
            );
            let numbers = NumbersShader::new(
                queue,
                render_pass,
                allocators.clone(),
                screen_size,

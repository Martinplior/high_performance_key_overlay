...
use sak_rs::{
    font::{
        FontFallbackList, LineLayout,
        layout::{LineLayoutLibrary, LineLayoutMetrics},
    },
    graphics::renderer::vulkan::{
        Allocators, PREMUL_ALPHA, command_builder::CommandBuilder, mipmap,
    },
};
use vulkano::{
...
use crate::key_overlay_core::{key_handler::KeyHandler, key_property::KeyProperty};

use super::shaders;

struct NumbersLayoutLibrary {
    map: ahash::HashMap<(char, u32), LineLayoutMetrics>,
}

impl NumbersLayoutLibrary {
    fn new(library: &impl LineLayoutLibrary, properties: &[KeyProperty]) -> Self {
        let map = properties
            .iter()
            .filter_map(|property| {
                if !property.key_counter.0 {
                    return None;
                }
                let counter = &property.key_counter.1;
                let font_size = counter.font_size;
                Some("0123456789".chars().map(move |ch| {
                    let metrics = library.metrics(ch, font_size);
                    ((ch, font_size.to_bits()), metrics)
                }))
            })
            .flatten()
            .collect();
...
impl LineLayoutLibrary for NumbersLayoutLibrary {
    fn metrics(&self, ch: char, px: f32) -> LineLayoutMetrics {
        let key = (ch, px.to_bits());
        self.map.get(&key).expect("unreachable").clone()
    }
}
...
pub struct NumbersShader {
    ...
    numbers_layout_library: NumbersLayoutLibrary,
    screen_size: [f32; 2],
...
impl NumbersShader {
    ...
    pub fn new(
        ...
        let numbers_layout_library = NumbersLayoutLibrary::new(&*fonts, key_properties);
    ...
    pub fn add_commands(
        ...
        let vertex_iter = key_handler
            .key_properties()
            .iter()
            .zip(key_handler.key_draw_caches())
            .enumerate()
            .filter_map(|(index, (property, cache))| {
                if !property.key_counter.0 {
                    return None;
                }
                let counter = &property.key_counter.1;
                let mut layout = LineLayout::new(counter.font_size);
